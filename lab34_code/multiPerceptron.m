function [classifiers posmiss negmiss] = multiPerceptron(pclass, nclass, n)
...
% negmiss - number (coefficient) of misclassified samples of nclass
  ...
  centers = combined_set(randi([1,rows(combined_set)],n), :);
  ...
  for cls =1:n
    ...
    do  
      ...
      classification = tset * classifiers(cls, :)'< 0;
      weighed_vec = tset.*weights;
      s = sum(reshape(weighed_vec(classification,:),[],columns(tset)),1);
